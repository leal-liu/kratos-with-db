...
func getQueriedProposals(
	...
	depositor, voter chainTypes.AccountID, status types.ProposalStatus, page, limit int,
) []types.Proposal {
...
func getQueriedDeposit(t *testing.T, ctx sdk.Context, cdc *codec.Codec, querier sdk.Querier, proposalID uint64, depositor chainTypes.AccountID) types.Deposit {
	query := abci.RequestQuery{
...
func getQueriedVote(t *testing.T, ctx sdk.Context, cdc *codec.Codec, querier sdk.Querier, proposalID uint64, voter chainTypes.AccountID) types.Vote {
	query := abci.RequestQuery{
...
func TestQuerier(t *testing.T) {
	...
	Convey("TestQueries", t, func() {
		...
		querier := govKeeper.NewQuerier(*keeper)

		oneCoins := chainTypes.NewCoins(chainTypes.NewCoin(stakingKeeper.BondDenom(ctx), sdk.NewInt(1)))
		consCoins := chainTypes.NewCoins(chainTypes.NewCoin(stakingKeeper.BondDenom(ctx), sdk.TokensFromConsensusPower(10)))
		...
		require.Equal(t, deposit4, deposits[0])
		...
		proposals := getQueriedProposals(t, ctx, app.Codec(), querier, chainTypes.EmptyAccountID(), chainTypes.EmptyAccountID(), types.StatusDepositPeriod, 1, 0)
		require.Len(t, proposals, 1)
		require.Equal(t, proposal1, proposals[0])

		// Only proposals #2 and #3 should be in Voting Period
		proposals = getQueriedProposals(t, ctx, app.Codec(), querier, chainTypes.EmptyAccountID(), chainTypes.EmptyAccountID(), types.StatusVotingPeriod, 1, 0)
		...
		proposals = getQueriedProposals(t, ctx, app.Codec(), querier, chainTypes.EmptyAccountID(), TestAddrs[0], types.StatusNil, 1, 0)
		require.Equal(t, proposal2, proposals[0])
		require.Equal(t, proposal3, proposals[1])
		...
		require.Equal(t, vote2, votes[1])
		require.Equal(t, vote3, votes[0])

		// Test query all proposals
		proposals = getQueriedProposals(t, ctx, app.Codec(), querier, chainTypes.EmptyAccountID(), chainTypes.EmptyAccountID(), types.StatusNil, 1, 0)
		require.Equal(t, proposal1, proposals[0])
		require.Equal(t, proposal2, proposals[1])
		require.Equal(t, proposal3, proposals[2])

		// Test query voted by TestAddrs[1]
		proposals = getQueriedProposals(t, ctx, app.Codec(), querier, chainTypes.EmptyAccountID(), TestAddrs[1], types.StatusNil, 1, 0)
		require.Equal(t, proposal3.ProposalID, proposals[0].ProposalID)

		// Test query deposited by TestAddrs[0]
		proposals = getQueriedProposals(t, ctx, app.Codec(), querier, TestAddrs[0], chainTypes.EmptyAccountID(), types.StatusNil, 1, 0)
		require.Equal(t, proposal1.ProposalID, proposals[0].ProposalID)

		// Test query deposited by addr2
		proposals = getQueriedProposals(t, ctx, app.Codec(), querier, TestAddrs[1], chainTypes.EmptyAccountID(), types.StatusNil, 1, 0)
	...
	Convey("TestQueries", t, func() {
		...
		// }
		proposal := types.NewProposal(TestProposal, 100, time.Now(), time.Now())
