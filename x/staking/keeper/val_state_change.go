...
func (k Keeper) BlockValidatorUpdates(ctx sdk.Context) []abci.ValidatorUpdate {
	...
	for _, dvPair := range matureUnbonds {
		balances, err := k.CompleteUnbonding(ctx, dvPair.DelegatorAccount, dvPair.ValidatorAccount)
		if err != nil {
			continue
		}
		...
		ctx.EventManager().EmitEvent(
			chainTypes.NewEvent(ctx,
				...
				sdk.NewAttribute(types.AttributeKeyDelegator, dvPair.DelegatorAccount.String()),
			),
		)
	}
	...
	for _, dvvTriplet := range matureRedelegations {
		balances, err := k.CompleteRedelegation(
			ctx,
			dvvTriplet.DelegatorAccount,
			dvvTriplet.ValidatorSrcAccount,
			dvvTriplet.ValidatorDstAccount,
		)
		if err != nil {
			continue
		}
		...
		ctx.EventManager().EmitEvent(
			chainTypes.NewEvent(ctx,
				...
				sdk.NewAttribute(types.AttributeKeySrcValidator, dvvTriplet.ValidatorSrcAccount.String()),
				sdk.NewAttribute(types.AttributeKeyDstValidator, dvvTriplet.ValidatorDstAccount.String()),
			),
		)
	}
...
func (k Keeper) ApplyAndReturnValidatorSetUpdates(ctx sdk.Context) (updates []abci.ValidatorUpdate) {
	...
	for count := 0; iterator.Valid() && count < int(maxValidators); iterator.Next() {
		...
		if !found || !bytes.Equal(oldPowerBytes, newPowerBytes) {
			...
			k.SetLastValidatorPower(ctx, valAccount, newPower)
		}
...
func (k Keeper) bondValidator(ctx sdk.Context, validator types.Validator) types.Validator {
	...
	validator = validator.UpdateStatus(stakingexport.Bonded)
	...
	// trigger hook
	k.AfterValidatorBonded(ctx, validator.GetConsAccount(), validator.OperatorAccount)
...
func (k Keeper) beginUnbondingValidator(ctx sdk.Context, validator types.Validator) types.Validator {
	...
	// sanity check
	if validator.Status != stakingexport.Bonded {
		panic(fmt.Sprintf("should not already be unbonded or unbonding, validator: %v\n", validator))
	}

	validator = validator.UpdateStatus(stakingexport.Unbonding)
	...
	// trigger hook
	k.AfterValidatorBeginUnbonding(ctx, validator.GetConsAccount(), validator.OperatorAccount)
...
func (k Keeper) completeUnbondingValidator(ctx sdk.Context, validator types.Validator) types.Validator {
	validator = validator.UpdateStatus(stakingexport.Unbonded)
	k.SetValidator(ctx, validator)
	return validator
}

// map of operator addresses to serialized power
type validatorsByAddr map[[types.AccountIDlen]byte][]byte
...
func (k Keeper) getLastValidatorsByAddr(ctx sdk.Context) validatorsByAddr {
	...
	for ; iterator.Valid(); iterator.Next() {
		var valAddr [types.AccountIDlen]byte
...
func sortNoLongerBonded(last validatorsByAddr) [][]byte {
	...
	for valAddrBytes := range last {
		valAddr := make([]byte, types.AccountIDlen)
