...
func TestNewSnowNode(t *testing.T) {

	_, err := NewSnowNode(0)
	if err != nil {
		t.Fatalf("error creating NewSnowNode, %s", err)
	}

	_, err = NewSnowNode(5000)
	if err == nil {
		t.Fatalf("no error creating NewSnowNode, %s", err)
	}
